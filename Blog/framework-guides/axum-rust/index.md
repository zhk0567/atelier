---
title: "Axum 官方指南：从入门到 Framework 子工程实战"
series: framework
category: Back-end
stack: Axum
repo_path: Back-end/Rust/Axum
...
# Axum 官方指南：从入门到 Framework 子工程实战
...
**Axum** 基于 Tower 与 Hyper，用 `Router` 组合路由与提取器。本子工程演示 health/info 与 `axum::serve`，默认 **http://127.0.0.1:3100/**，`service` 为 **framework-back-end-axum-guide**。
...
**Framework 约定**：`GET /api/health` 返回 `service: framework-back-end-axum-guide`；默认监听 **http://127.0.0.1:3100/**。与同仓 Node 示例对照时，请固定使用本文 PowerShell 路径与端口，便于脚本批量探活。呈现页 `public/index.html` 通过 `// 静态资源在 public/ 或 resources/` 编入二进制，部署时不必再单独拷贝静态目录。若你在内部 Wiki 引用本文，请同步贴上子工程仓库链接，避免同事只读到指南却找不到源码。
...
> **TIP**：本机并行跑多个 Go 示例时，用 `$env:PORT` 区分进程，避免与同仓 Actix Web（3110）、Warp（3112）等端口冲突。
...
在 PowerShell 中进入子工程目录并启动（首次会拉取 ``Axum` 依赖（见 Cargo.toml 或 build.sbt）`）：
...
「第一个 HTTP 服务」在这里体现为：**监听地址拼装 → `r.Run(addr)` 启动**。`Run` 内部创建 HTTP Server 并阻塞；出错时返回 `error`，子工程用 `log.Fatal` 打出原因后退出。默认 `HOST` 为 `127.0.0.1`，`PORT` 为 `3100`，因此本机在浏览器打开 `http://127.0.0.1:3100/` 应看到嵌入说明页；同时终端会打印「呈现页」与 `/api` 前缀的友好提示，方便新人对照路径自测。
...
从浏览器 `fetch` 调用 API 时，请从 `http://127.0.0.1:3100/` 打开呈现页，避免 `file://` 协议导致的跨域与安全限制。需要上传文件或表单时，应改用 `multipart` 绑定系列 API（本示例未覆盖，但 Context 路径一致）。响应头若由中间件与 Handler 同时设置，注意后者是否会覆盖前者，以及 CORS `ExposeHeaders` 是否列出了前端需要读取的自定义头。
...
子工程读取 **`HOST/PORT`**、**`HOST`**、**`PORT`**。若未设置 `HOST/PORT`，代码显式调用 `r := mux.NewRouter()`，保证本地开箱即用；当你在生产容器注入 `RUST_LOG=info` 时，将跳过这段默认赋值路径并采用 Release。`HOST` 默认为 `127.0.0.1`，适合本机开发；在服务器上常改为 `0.0.0.0` 以便容器端口映射或反代访问。`PORT` 默认为 **3100**，与 Framework 端口约定一致；在 PowerShell 可临时 `$env:PORT="3011"` 验证多实例编排。

除此之外，建议在团队层面约定「环境变量命名空间」：公共前缀可以避免与系统或其他语言进程冲突；敏感配置不进仓库，不进日志。若你把 `PORT` 交给平台注入，也要在 README 写明健康检查的完整 URL，避免文档写 `3100` 而实际跑在 `8080`。对多区域部署，有时还要区分 `HOST/PORT` 与真正的「业务环境」（staging、prod），二者不要混用一个变量承载所有语义，否则你在排查问题时很难判断到底是框架日志级别变了还是下游依赖地址变了。
...
```powershell
Invoke-RestMethod http://127.0.0.1:3100/api/health | ConvertTo-Json
Invoke-RestMethod http://127.0.0.1:3100/api/items | ConvertTo-Json
Invoke-RestMethod http://127.0.0.1:3100/api/demo/lifecycle | ConvertTo-Json
(Invoke-WebRequest http://127.0.0.1:3100/api/box/inner).Headers
```
...
理解 `Router::new().route(...)` 与 `IntoResponse` 即可跑通本示例。与同仓 Actix、Rocket 对比时，优先看「团队是否已标准化 Tokio 生态」。排障：`bind` 失败多为端口冲突；改 `PORT` 环境变量后 `cargo run`。
...
完整源码（`F:\Study\Framework\Back-end\Rust\Axum`，主入口 `src/main.rs`）：
...
- [Axum 文档](https://github.com/tokio-rs/axum)
- [Framework 子工程：Back-end/Rust/Axum](https://github.com/zhk0567/Framework/tree/main/Back-end/Rust/Axum)