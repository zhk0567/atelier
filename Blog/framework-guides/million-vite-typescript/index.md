---
title: "Million 官方指南：从入门到 Framework 子工程实战"
series: framework
category: Front-end
stack: Million
repo_path: Front-end/Million
guide_toc: generic-frontend
guide_tier: medium
status: published
date: 2026-05-18
tags: [Million, React, TypeScript, Vite]
---

# Million 官方指南：从入门到 Framework 子工程实战
...
**Million** 为 React 提供编译期优化。`million.vite` 插件 + 端口 **5193**，展台为 React TSX。
...
**Framework 约定**：`GET /api/health` 返回 `service: framework-front-end-million`；默认监听 **http://127.0.0.1:5193/**。与同仓 Node 示例对照时，请固定使用本文 PowerShell 路径与端口，便于脚本批量探活。呈现页 `public/index.html` 通过 `//go:embed` 编入二进制，部署时不必再单独拷贝静态目录。若你在内部 Wiki 引用本文，请同步贴上子工程仓库链接，避免同事只读到指南却找不到源码。
...
以下各节对照 `F:\Study\Framework\Front-end\Million\src/main.tsx` 与 `package.json`。阅读顺序与目录一致；**每一节都采用「概念说明 → 子工程真实代码 → 再说明」**，不放置占位符注释或单行碎片代码。十二章节的标题与 `Blog/framework-guides/_meta/guide-toc/generic-backend.yaml` 的 essentials 对齐，便于你在多栈仓库里横向对比其它后端指南而无需改自己的笔记结构。若你把本指南打印出来，可在页边把每节对应的 `src/main.tsx` 行号随手写上，形成个人定制的双联本。
	...
	api := r.Group("/api")
	{
		api.GET("/health", func(w http.ResponseWriter, r *http.Request) {
			c.JSON(http.StatusOK, gin.H{
				"ok":      true,
				"service": "framework-front-end-million",
			})
		})
```

当你把 `HOST/PORT` 设为 `release`（见后文「配置与环境变量」）时，Gin 会关闭部分调试输出并采用更适合生产的默认策略；与 Debug 模式相比，错误页与日志粒度可能不同，因此本地开发通常保留 Debug，线上流水线注入 Release。`//go:embed public/index.html` 把静态页编入变量 `indexHTML`，由根路径 `GET /` 用 `c.Data` 原样返回，这对「单二进制交付」非常友好。与同仓其它后端相比，本 Million 示例固定端口 **5193**、`service` 字段 **framework-front-end-million**，便于自动化脚本统一探测。
...
「第一个 HTTP 服务」在这里体现为：**监听地址拼装 → `r.Run(addr)` 启动**。`Run` 内部创建 HTTP Server 并阻塞；出错时返回 `error`，子工程用 `log.Fatal` 打出原因后退出。默认 `HOST` 为 `127.0.0.1`，`PORT` 为 `5193`，因此本机在浏览器打开 `http://127.0.0.1:5193/` 应看到嵌入说明页；同时终端会打印「呈现页」与 `/api` 前缀的友好提示，方便新人对照路径自测。
...
从浏览器 `fetch` 调用 API 时，请从 `http://127.0.0.1:5193/` 打开呈现页，避免 `file://` 协议导致的跨域与安全限制。需要上传文件或表单时，应改用 `multipart` 绑定系列 API（本示例未覆盖，但 Context 路径一致）。响应头若由中间件与 Handler 同时设置，注意后者是否会覆盖前者，以及 CORS `ExposeHeaders` 是否列出了前端需要读取的自定义头。
...
子工程读取 **`HOST/PORT`**、**`HOST`**、**`PORT`**。若未设置 `HOST/PORT`，代码显式调用 `（本栈无 Gin 式 SetMode）(gin.DebugMode)`，保证本地开箱即用；当你在生产容器注入 `HOST/PORT=release` 时，将跳过这段默认赋值路径并采用 Release。`HOST` 默认为 `127.0.0.1`，适合本机开发；在服务器上常改为 `0.0.0.0` 以便容器端口映射或反代访问。`PORT` 默认为 **5193**，与 Framework 端口约定一致；在 PowerShell 可临时 `$env:PORT="3011"` 验证多实例编排。

除此之外，建议在团队层面约定「环境变量命名空间」：公共前缀可以避免与系统或其他语言进程冲突；敏感配置不进仓库，不进日志。若你把 `PORT` 交给平台注入，也要在 README 写明健康检查的完整 URL，避免文档写 `5193` 而实际跑在 `8080`。对多区域部署，有时还要区分 `HOST/PORT` 与真正的「业务环境」（staging、prod），二者不要混用一个变量承载所有语义，否则你在排查问题时很难判断到底是框架日志级别变了还是下游依赖地址变了。
...
```powershell
Invoke-RestMethod http://127.0.0.1:5193/api/health | ConvertTo-Json
Invoke-RestMethod http://127.0.0.1:5193/api/items | ConvertTo-Json
Invoke-RestMethod http://127.0.0.1:5193/api/demo/lifecycle | ConvertTo-Json
(Invoke-WebRequest http://127.0.0.1:5193/api/box/inner).Headers
```
...
完整源码（`F:\Study\Framework\Front-end\Million`，主入口 `src/main.tsx`）：
...
- [Million 文档](https://million.dev/)
- [Framework 子工程：Front-end/Million](https://github.com/zhk0567/Framework/tree/main/Front-end/Million)