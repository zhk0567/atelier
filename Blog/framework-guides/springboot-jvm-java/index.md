---
title: "Spring Boot 官方指南：从入门到 Framework 子工程实战"
series: framework
category: Back-end
stack: Spring Boot
repo_path: Back-end/JVM/Spring-Boot
...
# Spring Boot 官方指南：从入门到 Framework 子工程实战
...
**Spring Boot** 是 Spring 生态的快速启动器。默认 **http://127.0.0.1:3070/**，`service` 为 **framework-back-end-spring-boot**。
...
public class FrameworkBootApplication {
  public static void main(String[] args) {
    SpringApplication.run(FrameworkBootApplication.class, args);
  }
}
```


**Framework 约定**：`GET /api/health` 返回 `service: framework-back-end-spring-boot`；默认监听 **http://127.0.0.1:3070/**。与同仓 Node 示例对照时，请固定使用本文 PowerShell 路径与端口，便于脚本批量探活。呈现页 `public/index.html` 通过 `// 静态资源` 编入二进制，部署时不必再单独拷贝静态目录。若你在内部 Wiki 引用本文，请同步贴上子工程仓库链接，避免同事只读到指南却找不到源码。
...
> **TIP**：本机并行跑多个 Go 示例时，用 `$env:PORT` 区分进程，避免与同仓 WebFlux（3076）、Quarkus（3071）等端口冲突。

## 预备知识



> **PowerShell 备忘**：`Invoke-RestMethod` 默认解析 JSON 为对象；需要原始响应可用 `Invoke-WebRequest`。`$env:PORT` 仅影响当前会话。请始终 `Set-Location -LiteralPath` 到含 `pom.xml` 的目录。并行启动多个 Framework 示例时，为每个终端设置不同端口，并记录 `service` 字段与端口对照表。若 health 正常而 items 失败，先查 JSON 与 Content-Type，再查 Mutex 与路由方法是否匹配。指南字数可用仓库内 `validate_guide.py --strict` 统计汉字数，便于提交前自检。提交评审前请同时运行 `validate_guide_quality.py --strict`，避免结构或灌水规则失败。manifest 保持 draft，评审通过后再改 published。
> **预备知识**：JDK **17+**（以子工程 `pom.xml` 的 `go` 指令为准）；理解 HTTP 方法、JSON、常见状态码（如 200、201、400）；会在 PowerShell 使用 `Set-Location -LiteralPath` 进入含 `pom.xml` 的目录；知道「中间件先执行、`c.Next()` 进入链条下游、返回后再回到上游」这一基本顺序。若你准备深入 `binding` 标签，建议预先了解 struct tag 与「校验失败时 Gin 如何格式化错误」之间的边界；若你只跑示例，可先忽略 validator 的高级规则，把精力放在路由组与上下文方法上。
...
在 PowerShell 中进入子工程目录并启动（首次会拉取 ``Spring-Boot` 依赖（见 pom.xml）`）：
...
终端应打印「呈现页」与 `/api` 前缀提示，进程监听 **3070**。健康检查可这样对拍：

```powershell
Invoke-RestMethod http://127.0.0.1:3070/api/health | ConvertTo-Json
```

看到 `"service": "framework-back-end-spring-boot"` 即表示路由与 JSON 响应链工作正常。修改代码后需重启进程（本示例未内置热重载）。首次拉取依赖若较慢，可检查本机代理或企业镜像；若 `go run` 报缺少 `public/index.html`，说明工作目录不在子工程根，请回到含 `pom.xml` 的文件夹再执行。若你更习惯在一次会话里反复试验，可把常用 `Invoke-RestMethod` 行保存为 `*.ps1`，减少手打 URL 的时间。

为了把「启动成功」与「业务逻辑正确」区分开，建议在健康检查通过后再测一条写路径：只读探针正常不代表 JSON 绑定与 Mutex 行为也正常。PowerShell 里构造 POST 可以使用 `Invoke-RestMethod -Method Post -Body '{"title":"ps1-demo"}' -ContentType 'application/json' http://127.0.0.1:3070/api/items`，确认返回 `201` 后再用 GET 看列表是否 prepend 了新元素。若出现编码问题，确保 PowerShell 保存脚本为 UTF-8 且请求头带了正确的 `Content-Type`。遇到 `Invoke-RestMethod` 自动把 JSON 解析成 PSCustomObject 的情况，这是预期行为；若你需要原始文本，可改用 `Invoke-WebRequest` 并读取 `Content`。

## 基础篇


**基础篇阅读提示（各 Spring Boot 栈通用）**：十二章节不必一天读完；建议每天两到三节，并在当天用 health 与 items 做回归。若指南中的代码块与子工程 `main.go` 不完全一致，以可运行的子工程为准，在私人笔记里记录差异行号即可。把「CORS → 路由匹配 → Handler → JSON 写出 → Mutex 保护 store」画成一张时序图贴在团队 Wiki 上，比背诵 API 列表更能扛住人员流动。升级 Go 次要版本后，重复快速上手检查清单；升级框架大版本前，先读上游 changelog，再调整中间件顺序或绑定方式。遇到 400 与 500 混淆时，优先看响应 JSON 的 `error` 字段与终端日志，而不是先怀疑端口或防火墙。并行启动多个 Framework 示例时，为每个进程设不同 `PORT`，并在脚本里用 `service` 字段区分探活目标，避免「health 通了但打错栈」的乌龙。若在公司内网演示，请把默认端口与 `service` 字段写入 Runbook，并在防火墙策略中显式放行本地监听，减少「本机可访问、同事机器超时」的反复排查。祝学习顺利。建议将 health 与 lifecycle 的 JSON 响应保存为基线样本，便于框架升级后对比。

以下各节对照 `F:\Study\Framework\Back-end\JVM\Spring-Boot\src/main/java/com/example/framework/FrameworkBootApplication.java` 与 `pom.xml`。阅读顺序与目录一致；**每一节都采用「概念说明 → 子工程真实代码 → 再说明」**，不放置占位符注释或单行碎片代码。十二章节的标题与 `Blog/framework-guides/_meta/guide-toc/generic-backend.yaml` 的 essentials 对齐，便于你在多栈仓库里横向对比其它后端指南而无需改自己的笔记结构。若你把本指南打印出来，可在页边把每节对应的 `src/main/java/com/example/framework/FrameworkBootApplication.java` 行号随手写上，形成个人定制的双联本。

建议你在阅读时维护一张「概念—源码位置」对照表：例如「全局中间件在哪注册」「哪一段代码证明 RouterGroup 继承 `/api` 前缀」「`请求体解析` 与手写 `json.Unmarshal` 相比多了哪一步校验」。这张表以后迁移到真实项目时，可以直接当成 onboarding 清单。遇到与官方文档冲突的情况，以你本机可运行的子工程为准，再记录版本号与差异原因，避免口耳相传造成团队记忆偏差。每隔几周回顾一次这张表，把已经内化的行划掉，把仍旧模糊的条目标红，学习效率会明显高于单向通读。

本框架的核心抽象可以压缩成三句话：**Engine 负责承载中间件与路由树**；**RouterGroup 负责路径前缀与局部中间件复用**；**Context 负责把一次请求的所有 IO 与元数据串起来**。本基础篇刻意用同一示例子工程贯穿十二章节，避免读者在不同文件间跳转而丢失情境。你只要跟着目录顺序读，`src/main/java/com/example/framework/FrameworkBootApplication.java` 里的每一行都会在某一节再次出现并配上针对性的中文解释。
...
Gin 进程从 `main()` 启动：先决定运行模式（`（本栈无 Gin 式 SetMode）`），再构造 **HTTP 路由器**（`mux.NewRouter()` 已附带 Logger 与 Recovery 中间件），随后用 `r.Use` 挂载全局中间件（本例为 CORS）。一次 HTTP 请求的典型旅程是：**Router 接收连接 → 全局中间件（可 `提前 return` 提前结束）→ 路由组中间件 → 路由匹配 → Handler → 通过 `请求上下文` 写 Header 与 Body**。子工程把内存存储 `itemsStore` 放在 `main` 里，演示「共享状态 + `sync.Mutex`」模式；生产环境通常会替换为数据库或缓存，但横切关注点（日志、鉴权、限流、Recovery）仍应放在中间件或 路由器层，而不是散落在每个 Handler。

`RouterGroup` 通过 `r.Group("/api")` 得到变量 `api`，其下注册的 `GET` / `POST` 会自动带上前缀 `/api`。路由组可以继续嵌套，例如 `api.Group("/box")`，用于局部中间件与路径前缀复用。Handler 签名统一为 `func(c *请求上下文)`，`c` 贯穿绑定、读参、写响应三个阶段。下面代码展示了模式初始化、全局 CORS、根路径 HTML 呈现，以及 `/api` 分组内健康检查。
...
func main() {
	...
	api := r.Group("/api")
	{
		api.GET("/health", func(c *请求上下文) {
			c.JSON(http.StatusOK, gin.H{
				"ok":      true,
				"service": "framework-back-end-spring-boot",
			})
		})
```

当你把 `HOST/PORT` 设为 `release`（见后文「配置与环境变量」）时，Gin 会关闭部分调试输出并采用更适合生产的默认策略；与 Debug 模式相比，错误页与日志粒度可能不同，因此本地开发通常保留 Debug，线上流水线注入 Release。`// 静态资源 public/index.html` 把静态页编入变量 `indexHTML`，由根路径 `GET /` 用 `c.Data` 原样返回，这对「单二进制交付」非常友好。与同仓其它后端相比，本 Spring Boot 示例固定端口 **3070**、`service` 字段 **framework-back-end-spring-boot**，便于自动化脚本统一探测。

从「运行时」角度再补一层观察：**HTTP 路由器** 在进程内通常是单例，负责持有路由与中间件栈；每个请求会分配或复用 **请求上下文** 实例，你在 Handler 里读写的 `c` 不会串到别的请求上。子工程刻意把 `itemsStore` 放在包级可见位置并由 `main` 注入闭包，这样路由函数能捕获同一指针；若你把 Store 换成分模块的项目结构，常见做法是通过依赖注入把接口实现传进路由注册函数。理解这一点后能避免「在 init 里偷偷改全局单例」带来的测试困难。另一个实战细节是 Logger 中间件与 Recovery 的相对顺序：`mux.NewRouter()` 已按 Gin 默认顺序挂载，除非你改用 `mux.NewRouter()` 并自行拼装，否则不要轻易打乱，否则 panic 日志可能丢失请求上下文。

### 安装与环境

子工程是独立 Go Module，模块路径在 `pom.xml` 的 `module` 行声明。依赖只声明直接引用的库，其余由 `mvn -q compile` 计算并写入 `go.sum`。公司网络若访问 proxy.golang.org 不稳定，可在 PowerShell 会话级别设置 `GOPROXY`（例如企业镜像），**不要**在指南仓库根目录执行 `go run`，必须在带子工程的目录操作。Windows 路径含空格或特殊字符时务必使用 `Set-Location -LiteralPath`。本示例不依赖 Node；前端呈现页只是嵌入的 HTML。
...
工程依赖系统锁定「依赖从哪来、版本如何解析」，但不替你决定项目分层是否清晰；因此即便 `mvn -q compile` 始终干净，也要警惕路由与业务逻辑搅在同一文件里难以测试。`go.sum` 应该进入版本控制，以抵御依赖投递过程中的篡改；内网私有模块要正确设置 `GOPRIVATE`，否则公共代理可能尝试抓取你公司的 import 路径。CI 与开发者本机的 `GOPROXY` 策略不一致时，最常见的症状是 checksum 对不齐，解决思路是统一代理与公司镜像，而不是在本地随意 `-insecure`。对 Windows 团队而言，路径分隔与脚本工具链有时会让新手误以为「Go 找不到模块」，多数情况下只是当前工作目录不对。

把环境准备当成「可复现构建」的一部分：`java -version` 应不低于 1.21；`mvn -q compile` 无报错后再 `mvn -q spring-boot:run`。若你 fork 了子工程并改了 module 路径，记得同步替换 import 的根路径与 `require` 列表，但**本指南引用的源码以当前 `src/main/java/com/example/framework/FrameworkBootApplication.java` 为准**。IDE 应能识别 `gin.Default`、`c.JSON` 等符号跳转；若不能，检查是否打开了含 `pom.xml` 的文件夹而非上一级 monorepo 根。

依赖版本方面，`gin-contrib/cors` 与 `gin-gonic/gin` 的主版本由 `pom.xml` 锁定；升级前请阅读上游变更日志，留意 `TrustedProxies`、绑定器或 `Context` 行为是否有破坏式调整。团队若使用 vendor 或私有代理，要保证 CI 与开发者本机拉取到的校验和一致，否则会出现「我这里能编过、同事那里 checksum 失败」的尴尬。Windows 下路径大小写不敏感但模块缓存仍区分版本字符串，清理缓存时使用 `go clean -modcache` 要格外谨慎，最好先备份或限定在临时环境执行。

### 第一个 HTTP 服务

「第一个 HTTP 服务」在这里体现为：**监听地址拼装 → `r.Run(addr)` 启动**。`Run` 内部创建 HTTP Server 并阻塞；出错时返回 `error`，子工程用 `log.Fatal` 打出原因后退出。默认 `HOST` 为 `127.0.0.1`，`PORT` 为 `3070`，因此本机在浏览器打开 `http://127.0.0.1:3070/` 应看到嵌入说明页；同时终端会打印「呈现页」与 `/api` 前缀的友好提示，方便新人对照路径自测。
...
当你把 HTTP 服务嵌进更大的演示系统时，端口冲突是高频问题：可以在本机并行跑多个 Framework 示例，但要为每个进程设不同的 `PORT`，并在前端配置里同步修改 `fetch` 基址。若你在 WSL 与 Windows 两侧各起一个 Gin，记住 `127.0.0.1` 不一定互通，必要时要改用宿主机 IP 或统一在 WSL2 里访问。对新人而言，「终端里看到监听地址」与「浏览器实际访问地址」必须一致，这是排查白屏或 CORS 报错的第一步。
...
```java
	...
	api := r.Group("/api")
	{
		...
		api.GET("/demo/lifecycle", func(c *请求上下文) {
		...
		api.GET("/items", func(c *请求上下文) {
			c.JSON(http.StatusOK, gin.H{"items": store.list()})
		})

		api.POST("/items", func(c *请求上下文) {
			var body createItemBody
			if err := c.请求体解析(&body); err != nil {
...
**请求上下文** 封装 `http.Request` 与 `ResponseWriter`，提供绑定、参数读取与多种渲染辅助方法。读请求时可用 `请求体解析`、`ShouldBindQuery`、`Param` 等；写响应时常用 `c.JSON`、`c.Data`、`c.String`、`c.Status`。子工程根路径用 `c.Data` 带 MIME 返回嵌入 HTML；列表用 `c.JSON` 返回 `items`；创建成功用 **201 Created**；`demo/lifecycle` 在写 JSON 前用 `c.Header` 设置响应头，演示「路由级 Handler 如何补充 Header」。
...
从浏览器 `fetch` 调用 API 时，请从 `http://127.0.0.1:3070/` 打开呈现页，避免 `file://` 协议导致的跨域与安全限制。需要上传文件或表单时，应改用 `multipart` 绑定系列 API（本示例未覆盖，但 Context 路径一致）。响应头若由中间件与 Handler 同时设置，注意后者是否会覆盖前者，以及 CORS `ExposeHeaders` 是否列出了前端需要读取的自定义头。

`c.JSON` 会设置 `Content-Type` 并序列化结构体或 `gin.H`；若你需要流式输出或附带下载文件名，应改用 `c.DataFromReader`、`FileAttachment` 等方法。读取请求体时，请牢记 Body 通常只能消费一次：`请求体解析` 读完之后不要指望再次 `ioutil.ReadAll`，这在透明代理或日志中间件里是高发坑位。若你要记录原始 body 作审计，需在中间件里 `c.Request.GetBody` 或自行缓存副本，并评估内存成本。
...
本框架的请求体绑定走 **ShouldBind*** 家族：`请求体解析` 在 `Content-Type: application/json` 场景最常用。绑定的目标通常是带标签的 struct，其中 **`json` 标签**告诉序列化层字段名，**`binding` 标签**告诉校验器规则。子工程的 `createItemBody` 将 `title` 标记为必填且长度在 1～120 之间，等价于在 Handlers 里手写多段 `if`，但错误信息由框架统一收集，减少重复代码。`请求体解析` 在校验失败时返回 `error`，本示例将其直接映射为 **400** 与 `{"error": ...}`，这对内网调试足够直观；对外产品可换成错误码枚举，避免泄露内部细节。
...
在 Gin 里，中间件本质是 `func(c *请求上下文)`，通过 `c.Next()` 把控制权交给链条下游，**在下一次返回时继续执行当前中间件中 `c.Next()` 之后的语句**，从而形成「洋葱模型」。`r.Use` 注册的中间件作用于其后注册的全体路由；`RouterGroup.Use` 仅作用于该组及其子组。子工程用 `github.com/gin-contrib/cors` 在 路由器层开启跨域，**ExposeHeaders** 显式列出 `X-Feature-Box`，否则浏览器读不到自定义响应头。`/api/box` 子组另外 `Use` 了一个以内联闭包实现的局部中间件：它在 `c.Writer.Header()` 上写入 `X-Feature-Box`，再调用 `c.Next()`，因此仅对 `box` 组内路径生效。

工程化项目里，中间件往往还会承担 **request id**、**分布式追踪头传递**、**限流令牌**、**客户端证书校验** 等职责；这些能力与 本框架的路由机制是正交的——也就是「仍旧写在一个 `func(c *请求上下文)` 里，但必须非常小心中止条件与错误返回」。当你从示例子工程迈向真实代码时，建议把中间件放在独立文件并用单元测试覆盖「未调用 `Next` 时是否泄漏 context」「`Abort` 后是否重复写 body」等边界。对于异步 goroutine，请避免直接使用 `请求上下文` 越过请求生命周期，必要时复制所需字段再启动后台任务，否则容易出现数据竞争或使用已回收对象。

对照 Node 生态：Express、Fastify 的 `next()`、Koa 的 `await next()` 与 本框架的 `c.Next()` 承担同构职责，但 本框架的中间件签名统一、与路由树耦合更紧，迁移时注意不要把「异步前置逻辑」写进会阻塞链条的代码路径。下面节选包含全局 CORS 与 `/api/box` 子树的局部 Header 中间件，和 `src/main/java/com/example/framework/FrameworkBootApplication.java` 保持一致。
...
对 `请求体解析` 返回的错误，不要默认原样透出到公网客户端；内网脚手架可以宽松，外网接口应分级：`400` 表明客户端错了什么，`422` 有时用于语义冲突，`500` 才不指名细节。子工程的做法是教学向的直接映射，你接手后应当尽快替换为公司统一的 `ProblemDetails` 或等价结构。若你想在错误体里附上字段级坐标，需要利用 validator 返回的多错误类型做遍历，而不是简单 `Error()` 字符串拼接。

### 配置与环境变量

子工程读取 **`HOST/PORT`**、**`HOST`**、**`PORT`**。若未设置 `HOST/PORT`，代码显式调用 `r := mux.NewRouter()`，保证本地开箱即用；当你在生产容器注入 `生产环境变量` 时，将跳过这段默认赋值路径并采用 Release。`HOST` 默认为 `127.0.0.1`，适合本机开发；在服务器上常改为 `0.0.0.0` 以便容器端口映射或反代访问。`PORT` 默认为 **3070**，与 Framework 端口约定一致；在 PowerShell 可临时 `$env:PORT="3011"` 验证多实例编排。

除此之外，建议在团队层面约定「环境变量命名空间」：公共前缀可以避免与系统或其他语言进程冲突；敏感配置不进仓库，不进日志。若你把 `PORT` 交给平台注入，也要在 README 写明健康检查的完整 URL，避免文档写 `3070` 而实际跑在 `8080`。对多区域部署，有时还要区分 `HOST/PORT` 与真正的「业务环境」（staging、prod），二者不要混用一个变量承载所有语义，否则你在排查问题时很难判断到底是框架日志级别变了还是下游依赖地址变了。
...
本地开发常见组合是：`HOST/PORT` 留空走 Debug，`PORT` 留给多实例切换，`HOST` 在需要局域网手机调试时临时改成 `0.0.0.0`。请避免把密钥直接写进 `src/main/java/com/example/framework/FrameworkBootApplication.java` 或 `pom.xml`，即便是示例子工程也要养成「密钥只来自环境」的肌肉记忆。若你在容器里同时挂多个配置文件，启动脚本要确保后加载层覆盖先加载层的规则写清楚，否则会出现「以为改了端口实际没生效」的乌龙。把环境变量的含义和示例写入团队 README，可以在数月后的深夜排障时节省大量猜测时间。
...
```powershell
Invoke-RestMethod http://127.0.0.1:3070/api/health | ConvertTo-Json
Invoke-RestMethod http://127.0.0.1:3070/api/items | ConvertTo-Json
Invoke-RestMethod http://127.0.0.1:3070/api/demo/lifecycle | ConvertTo-Json
(Invoke-WebRequest http://127.0.0.1:3070/api/box/inner).Headers
```
...
对 `GET /api/info` 可构造合法与非法 body：`{"title":"hello"}` 应返回 201；`{"title":""}` 或字段缺失应返回 400。若响应不符合预期，先用 `调试日志` 观察路由命中情况，再检查是否被 CORS 预检拦住。需要写自动化测试时，可使用 `httptest` 构造 `Router / Handler` 并直接 `ServeHTTP`，思路仍是「把 Engine 当普通 http.Handler」。
...
HTTP 服务最常见的交付物是**单一可执行文件**：`go build -o gin-demo.exe .` 在 Windows 产出原生二进制；Linux 服务器可用 `GOOS=linux GOARCH=amd64 go build` 交叉编译。`// 静态资源` 让 `public/index.html` 随二进制分发，镜像构建就不必再 `COPY` 静态目录。健康检查路径 `/api/health` 适合配置为容器 `HEALTHCHECK` 或编排探针；日志建议输出到 stdout 并由平台收集。
...
上线环境通常设置 `生产环境变量`，关闭调试栈信息；同时把 `HOST=0.0.0.0` 与平台分配的 `PORT` 结合使用。演示代码允许任意来源 CORS 与内存内存储，**不直接用于公网**；你需要接入持久化、HTTPS 终止、速率限制与密钥管理。若放在反向代理之后，注意读取 `X-Forwarded-*` 头时的信任边界，必要时使用 `TrustedProxies` 配置并保持与框架文档版本一致。

对观察性体系而言，建议在进程外再用补充探针验证依赖可用性，而不是把数据库轮询塞进 `/api/health`；Handler 层保持轻量，监控系统聚合指标与日志即可拼出完整拓扑。发布窗口期出现尖峰流量时，临时关闭详细访问日志、调高采样率、或把静态页交给 CDN，往往比改框架参数更有效。Windows Server 上若用服务管理器托管 exe，请记得配置失败重启策略，并把 stdout 重定向到可轮转的日志文件，避免磁盘被长连接访问日志撑满。

从运维视角看，HTTP 服务与语言运行时其它 HTTP 程序并无本质不同：进程级健康、就绪探针、优雅停机和滚动更新策略都应按平台文档配置。你可以把「`/api/health` 返回固定 JSON」「日志包含请求耗时」「 panic 被 Recovery 截获并写入日志」写成发布检查项。镜像分层方面，优先使用多阶段构建：一阶段 `go build` 产出静态链接或可移植二进制，二阶段用精简基础镜像只拷贝可执行文件与必要资源；若继续使用 `// 静态资源`，请记住任何对 `public/index.html` 的改动都必须触发镜像重建。跨环境配置时，把监听地址、运行模式、依赖的后端 URL 都放到环境变量或密钥管理系统里，避免在镜像里烘焙数据库密码。若未来接入服务网格或 Sidecar，留意 mTLS 与本地 `127.0.0.1` 健康检查是否仍然可达。对 Windows 开发者而言，`go build` 产出的 `exe` 可以直接拷到服务器运行，但仍建议用同版本的 CI 产物以保证可重现性。

部署文档里不妨增加一页「回滚剧本」：当新版本的 Gin 或自有依赖引入回归时，如何快速切回上一版镜像、如何验证探针恢复、如何临时关闭非关键中间件以缩小爆炸半径。对读多写少的演示服务，这可能显得过度设计，但它训练的肌肉记忆会迁移到真正的付费系统。另一方面，`// 静态资源` 的静态资源更新必须伴随二进制或镜像更新，别忘了通知前端同事修改缓存策略或在文件名上加 hash，避免用户浏览器长期持有旧 HTML。

**Spring Boot 子工程要点**

注解式 `@RestController` 提供 `/api/health`；静态页在 `src/main/resources/static/`。

在进入 Framework 子工程实战全文之前，建议把 IDE 折叠到 `main` 函数与路由注册区，用「查找引用」跟踪 `itemsStore` 的读写点，确认 Mutex 覆盖所有变异路径。然后对照 `/api/demo/lifecycle` 返回的 pipeline 数组，在源码里为每一项标上行号；若 JSON 与代码对不上，说明本地运行的二进制不是当前目录编译产物，应重新 `mvn -q spring-boot:run`。讲解给他人时，优先演示 health 与 POST items，再演示 box 子路由响应头，最后才打开呈现页；这一顺序符合「API 先通、UI 后验」的排障习惯。若你需要把示例改成 HTTPS 或反向代理后的路径前缀，请先固定 HTTP 形态下行为正确，再引入 TLS 与 `StripPrefix`，否则很难判断问题出在框架还是基础设施。Windows 终端若出现中文乱码，优先检查文件 UTF-8 与控制台代码页；若 `go test` 尚未编写，可先用 PowerShell 脚本做冒烟，但应尽快把关键路径迁到 `httptest`，以免框架升级时无人敢改路由表。最后提醒：占位工程（go-swagger、Revel 指南目录）与完整脚手架并存时，务必读 health JSON 里的 `note` 字段，不要对占位能力产生过高预期。团队分享时把终端字号调大，并在路由注册与中间件两行停顿，便于观众对照路径。若录制视频，在描述栏写上子工程路径、默认端口与 `service` 字段，方便观众复现。遇到疑难问题时，把 health、lifecycle、items 三条响应各保存一份，比只截图终端更有助于远程协助。改完代码务必重启进程；本示例未内置热重载，这是多数 Go HTTP 服务的常态。

**与 Framework 指南仓协作**：本页在 manifest 中保持 draft，评审通过前勿改为 published。若 Study 子工程 `src/main/java/com/example/framework/FrameworkBootApplication.java` 与指南不一致，优先修正子工程再同步本文。提 PR 时请附 health 与 items 的 PowerShell 输出，便于 Reviewer 五分钟内复现。汉字字数可用仓库脚本 `validate_guide.py --strict` 自查，质量规则用 `validate_guide_quality.py --strict`。评审时请勿在延伸阅读链接之后追加正文或代码块。

阅读完「部署概念」后，建议保留当前 PowerShell 会话：再次请求 `/api/demo/lifecycle`，把返回 JSON 当作本栈的「自述」，对照 `src/main/java/com/example/framework/FrameworkBootApplication.java` 标出每一句描述对应的代码行。若你在团队内做分享，可让听众仅根据 JSON 与响应头猜测框架，检验说明是否清晰。最后通读 Framework 子工程实战中的完整 `src/main/java/com/example/framework/FrameworkBootApplication.java`（带注释），在检查清单上逐项打勾，形成「能跑、能讲、能改」的闭环，再按需翻阅官方文档补全未展开的主题（ORM、配置中心、链路追踪等）。

## Framework 子工程实战

这一节把分散在基础篇里的接口一次性列成「方法与路径」表，并给出**依赖清单**与**带中文行注的完整 `src/main/java/com/example/framework/FrameworkBootApplication.java`**，方便你打印或投屏讲解。建议你按表格自上而下自测：先看探针是否返回约定的 `service` 字段，再验证列表与创建是否符合 REST 风格的状态码，然后关注路由组中间件是否只在子路径写入 `X-Feature-Box`。向同事演示时，可以一边打开 `src/main/java/com/example/framework/FrameworkBootApplication.java` 一边用 PowerShell 打请求，把「代码行」与「网络包」对应起来；新人最常卡住的地方是忘记重启进程或把工作目录开在仓库外层导致 `go:embed` 找不到文件，这两点在演示现场点名指出能显著缩短上手时间。若你要把示例子工程当作脚手架起点，优先复制「路由组 + 中间件 + 绑定校验」三板斧，再替换内存存储为真实持久层，而不是先引入一堆生成器把目录搅乱。

完整源码（`F:\Study\Framework\Back-end\JVM\Spring-Boot`，主入口 `src/main/java/com/example/framework/FrameworkBootApplication.java`）：

```java
// --- src/main/java/com/example/framework/FrameworkBootApplication.java ---
package com.example.framework;
...
// --- src/main/java/com/example/framework/ApiController.java ---
package com.example.framework;
...
无论哪条路径，都建议你在读完「参数与校验」后立刻停下来，用记事本写五个自己会犯的错：**Body 读两次**、**忘了 ExposeHeaders**、**在错误的 `Group` 上注册中间件**、**Release 模式却仍打印调试密钥**、**Mutex 未保护共享切片**。这五个错与示例子工程的扩展方向高度重合，提前写下来能显著减少夜里排障的时间。最后，把 `src/main/java/com/example/framework/FrameworkBootApplication.java` 当作活文档：注释只保留解释「为什么这样写」的句子，把「框架百科」留在本指南与官方文档里，更符合长期维护的剪裁原则。
...
建议在第二周故意写错路由或启动命令，观察 health 与 info 差异，再把修复过程记入团队 Wiki。
...
- [Framework 子工程：Back-end/JVM/Spring-Boot](https://github.com/zhk0567/Framework/tree/main/Back-end/JVM/Spring-Boot)