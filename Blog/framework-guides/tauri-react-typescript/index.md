---
title: "Tauri 官方指南：从入门到 Framework 子工程实战"
series: framework
category: Front-end
stack: Tauri
repo_path: Front-end/Tauri
guide_toc: generic-frontend
guide_tier: medium
status: published
date: 2026-05-18
tags: [Tauri, React, Rust, TypeScript, Vite]
---

# Tauri 官方指南：从入门到 Framework 子工程实战
...
**Tauri 2** 用系统 WebView + Rust 原生层。本子工程 React 前端默认 **http://localhost:1420/**，示例 **`greet`** command 见 `src-tauri/src/lib.rs`。
...
**Framework 约定**：`GET /api/health` 返回 `service: framework-front-end-tauri`；默认监听 **http://127.0.0.1:1420/**。与同仓 Node 示例对照时，请固定使用本文 PowerShell 路径与端口，便于脚本批量探活。呈现页 `public/index.html` 通过 `//go:embed` 编入二进制，部署时不必再单独拷贝静态目录。若你在内部 Wiki 引用本文，请同步贴上子工程仓库链接，避免同事只读到指南却找不到源码。
...
> **TIP**：本机并行跑多个 Go 示例时，用 `$env:PORT` 区分进程，避免与同仓 Electron、Capacitor等端口冲突。
...
> **预备知识**：Node **20+**、Rust **1.70+**（以子工程 `package.json` 的 `go` 指令为准）；理解 HTTP 方法、JSON、常见状态码（如 200、201、400）；会在 PowerShell 使用 `Set-Location -LiteralPath` 进入含 `package.json` 的目录；知道「中间件先执行、`c.Next()` 进入链条下游、返回后再回到上游」这一基本顺序。若你准备深入 `binding` 标签，建议预先了解 struct tag 与「校验失败时 Gin 如何格式化错误」之间的边界；若你只跑示例，可先忽略 validator 的高级规则，把精力放在路由组与上下文方法上。
...
看到 `"service": "framework-front-end-tauri"` 即表示路由与 JSON 响应链工作正常。修改代码后需重启进程（本示例未内置热重载）。首次拉取依赖若较慢，可检查本机代理或企业镜像；若 `go run` 报缺少 `public/index.html`，说明工作目录不在子工程根，请回到含 `package.json` 的文件夹再执行。若你更习惯在一次会话里反复试验，可把常用 `Invoke-RestMethod` 行保存为 `*.ps1`，减少手打 URL 的时间。
	...
	api := r.Group("/api")
	{
		api.GET("/health", func(w http.ResponseWriter, r *http.Request) {
			c.JSON(http.StatusOK, gin.H{
				"ok":      true,
				"service": "framework-front-end-tauri",
			})
		})
```

当你把 `HOST/PORT` 设为 `release`（见后文「配置与环境变量」）时，Gin 会关闭部分调试输出并采用更适合生产的默认策略；与 Debug 模式相比，错误页与日志粒度可能不同，因此本地开发通常保留 Debug，线上流水线注入 Release。`//go:embed public/index.html` 把静态页编入变量 `indexHTML`，由根路径 `GET /` 用 `c.Data` 原样返回，这对「单二进制交付」非常友好。与同仓其它后端相比，本 Tauri 示例固定端口 **1420**、`service` 字段 **framework-front-end-tauri**，便于自动化脚本统一探测。
...
`invoke` 与 `#[tauri::command]` 是 IPC 主线；发布前配置 `tauri.conf.json`。
...
- [Framework 子工程：Front-end/Tauri](https://github.com/zhk0567/Framework/tree/main/Front-end/Tauri)