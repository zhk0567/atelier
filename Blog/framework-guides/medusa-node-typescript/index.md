---
title: "Medusa 官方指南：从入门到 Framework 子工程实战"
series: framework
category: Back-end
stack: Medusa
repo_path: Back-end/Node/Medusa
guide_toc: generic-backend
guide_tier: medium
status: published
date: 2026-05-18
tags: [Medusa, TypeScript, Node]
---

# Medusa 官方指南：从入门到 Framework 子工程实战
...
**Medusa 电商后端；本子工程为 HTTP 形态占位，完整能力需 Medusa CLI。** 默认 **http://127.0.0.1:3115/**，`service` 为 **framework-back-end-medusa-guide**。
...
**Framework 约定**：`GET /api/health` 返回 `service: framework-back-end-medusa-guide`；默认监听 **http://127.0.0.1:3115/**。与同仓 Node 示例对照时，请固定使用本文 PowerShell 路径与端口，便于脚本批量探活。呈现页 `public/index.html` 通过 `// 静态资源在 public/` 编入二进制，部署时不必再单独拷贝静态目录。若你在内部 Wiki 引用本文，请同步贴上子工程仓库链接，避免同事只读到指南却找不到源码。
...
在 PowerShell 中进入子工程目录并启动（首次会拉取 ``Medusa` 依赖（见 package.json）`）：
...
终端应打印「呈现页」与 `/api` 前缀提示，进程监听 **3115**。健康检查可这样对拍：

```powershell
Invoke-RestMethod http://127.0.0.1:3115/api/health | ConvertTo-Json
```

看到 `"service": "framework-back-end-medusa-guide"` 即表示路由与 JSON 响应链工作正常。修改代码后需重启进程（本示例未内置热重载）。首次拉取依赖若较慢，可检查本机代理或企业镜像；若 `go run` 报缺少 `public/index.html`，说明工作目录不在子工程根，请回到含 `package.json` 的文件夹再执行。若你更习惯在一次会话里反复试验，可把常用 `Invoke-RestMethod` 行保存为 `*.ps1`，减少手打 URL 的时间。

为了把「启动成功」与「业务逻辑正确」区分开，建议在健康检查通过后再测一条写路径：只读探针正常不代表 JSON 绑定与 Mutex 行为也正常。PowerShell 里构造 POST 可以使用 `Invoke-RestMethod -Method Post -Body '{"title":"ps1-demo"}' -ContentType 'application/json' http://127.0.0.1:3115/api/items`，确认返回 `201` 后再用 GET 看列表是否 prepend 了新元素。若出现编码问题，确保 PowerShell 保存脚本为 UTF-8 且请求头带了正确的 `Content-Type`。遇到 `Invoke-RestMethod` 自动把 JSON 解析成 PSCustomObject 的情况，这是预期行为；若你需要原始文本，可改用 `Invoke-WebRequest` 并读取 `Content`。
...
以下各节对照 `F:\Study\Framework\Back-end\Node\Medusa\src/server.ts` 与 `package.json`。阅读顺序与目录一致；**每一节都采用「概念说明 → 子工程真实代码 → 再说明」**，不放置占位符注释或单行碎片代码。十二章节的标题与 `Blog/framework-guides/_meta/guide-toc/generic-backend.yaml` 的 essentials 对齐，便于你在多栈仓库里横向对比其它后端指南而无需改自己的笔记结构。若你把本指南打印出来，可在页边把每节对应的 `src/server.ts` 行号随手写上，形成个人定制的双联本。
...
当你把 `HOST/PORT` 设为 `release`（见后文「配置与环境变量」）时，Gin 会关闭部分调试输出并采用更适合生产的默认策略；与 Debug 模式相比，错误页与日志粒度可能不同，因此本地开发通常保留 Debug，线上流水线注入 Release。`// 静态资源在 public/ public/index.html` 把静态页编入变量 `indexHTML`，由根路径 `GET /` 用 `c.Data` 原样返回，这对「单二进制交付」非常友好。与同仓其它后端相比，本 Node 示例固定端口 **3115**、`service` 字段 **framework-back-end-medusa-guide**，便于自动化脚本统一探测。
...
「第一个 HTTP 服务」在这里体现为：**监听地址拼装 → `r.Run(addr)` 启动**。`Run` 内部创建 HTTP Server 并阻塞；出错时返回 `error`，子工程用 `log.Fatal` 打出原因后退出。默认 `HOST` 为 `127.0.0.1`，`PORT` 为 `3115`，因此本机在浏览器打开 `http://127.0.0.1:3115/` 应看到嵌入说明页；同时终端会打印「呈现页」与 `/api` 前缀的友好提示，方便新人对照路径自测。
...
从浏览器 `fetch` 调用 API 时，请从 `http://127.0.0.1:3115/` 打开呈现页，避免 `file://` 协议导致的跨域与安全限制。需要上传文件或表单时，应改用 `multipart` 绑定系列 API（本示例未覆盖，但 Context 路径一致）。响应头若由中间件与 Handler 同时设置，注意后者是否会覆盖前者，以及 CORS `ExposeHeaders` 是否列出了前端需要读取的自定义头。
...
子工程读取 **`HOST/PORT`**、**`HOST`**、**`PORT`**。若未设置 `HOST/PORT`，代码显式调用 `（本栈无 Gin 式 SetMode）(调试模式)`，保证本地开箱即用；当你在生产容器注入 `NODE_ENV=production` 时，将跳过这段默认赋值路径并采用 Release。`HOST` 默认为 `127.0.0.1`，适合本机开发；在服务器上常改为 `0.0.0.0` 以便容器端口映射或反代访问。`PORT` 默认为 **3115**，与 Framework 端口约定一致；在 PowerShell 可临时 `$env:PORT="3011"` 验证多实例编排。

除此之外，建议在团队层面约定「环境变量命名空间」：公共前缀可以避免与系统或其他语言进程冲突；敏感配置不进仓库，不进日志。若你把 `PORT` 交给平台注入，也要在 README 写明健康检查的完整 URL，避免文档写 `3115` 而实际跑在 `8080`。对多区域部署，有时还要区分 `HOST/PORT` 与真正的「业务环境」（staging、prod），二者不要混用一个变量承载所有语义，否则你在排查问题时很难判断到底是框架日志级别变了还是下游依赖地址变了。
...
```powershell
Invoke-RestMethod http://127.0.0.1:3115/api/health | ConvertTo-Json
Invoke-RestMethod http://127.0.0.1:3115/api/items | ConvertTo-Json
Invoke-RestMethod http://127.0.0.1:3115/api/demo/lifecycle | ConvertTo-Json
(Invoke-WebRequest http://127.0.0.1:3115/api/box/inner).Headers
```
...
Medusa 电商后端；本子工程为 HTTP 形态占位，完整能力需 Medusa CLI。 默认 **3115**，`service` 为 **framework-back-end-medusa-guide**。请先 `npm install` 再 `npm run start`，用 `Invoke-RestMethod http://127.0.0.1:3115/api/health` 探活。
...
完整源码（`F:\Study\Framework\Back-end\Node\Medusa`，主入口 `src/server.ts`）：
...
- [Medusa 文档](https://medusajs.com/)
- [Framework 子工程：Back-end/Node/Medusa](https://github.com/zhk0567/Framework/tree/main/Back-end/Node/Medusa)