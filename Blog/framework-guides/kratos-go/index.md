---
title: "Kratos 官方指南：从入门到 Framework 子工程实战"
series: framework
category: Back-end
stack: Kratos
repo_path: Back-end/Go/Kratos
...
# Kratos 官方指南：从入门到 Framework 子工程实战
...
**Kratos** 是面向微服务的 Go 框架：本子工程使用 **transport/http** 的 `khttp.NewServer`，在标准 `HandleFunc` 之上挂路由，并与 `kratos.App` 统一生命周期。默认 **http://127.0.0.1:3009/**，`service` 为 **framework-back-end-kratos**。启动时 `app.Run()` 处理信号与优雅退出。
...
**Framework 约定**：`GET /api/health` 返回 `service: framework-back-end-kratos`；默认监听 **http://127.0.0.1:3009/**。与同仓 Node 示例对照时，请固定使用本文 PowerShell 路径与端口，便于脚本批量探活。呈现页 `public/index.html` 通过 `//go:embed` 编入二进制，部署时不必再单独拷贝静态目录。若你在内部 Wiki 引用本文，请同步贴上子工程仓库链接，避免同事只读到指南却找不到源码。
...
> **TIP**：本机并行跑多个 Go 示例时，用 `$env:PORT` 区分进程，避免与同仓 go-zero（3008）、Gin（3002）等端口冲突。
...
在 PowerShell 中进入子工程目录并启动（首次会拉取 ``Kratos` 依赖（见 go.mod）`）：
...
终端应打印「呈现页」与 `/api` 前缀提示，进程监听 **3009**。健康检查可这样对拍：

```powershell
Invoke-RestMethod http://127.0.0.1:3009/api/health | ConvertTo-Json
```

看到 `"service": "framework-back-end-kratos"` 即表示路由与 JSON 响应链工作正常。修改代码后需重启进程（本示例未内置热重载）。首次拉取依赖若较慢，可检查本机代理或企业镜像；若 `go run` 报缺少 `public/index.html`，说明工作目录不在子工程根，请回到含 `go.mod` 的文件夹再执行。若你更习惯在一次会话里反复试验，可把常用 `Invoke-RestMethod` 行保存为 `*.ps1`，减少手打 URL 的时间。

为了把「启动成功」与「业务逻辑正确」区分开，建议在健康检查通过后再测一条写路径：只读探针正常不代表 JSON 绑定与 Mutex 行为也正常。PowerShell 里构造 POST 可以使用 `Invoke-RestMethod -Method Post -Body '{"title":"ps1-demo"}' -ContentType 'application/json' http://127.0.0.1:3009/api/items`，确认返回 `201` 后再用 GET 看列表是否 prepend 了新元素。若出现编码问题，确保 PowerShell 保存脚本为 UTF-8 且请求头带了正确的 `Content-Type`。遇到 `Invoke-RestMethod` 自动把 JSON 解析成 PSCustomObject 的情况，这是预期行为；若你需要原始文本，可改用 `Invoke-WebRequest` 并读取 `Content`。
...
以下各节对照 `F:\Study\Framework\Back-end\Go\Kratos\main.go` 与 `go.mod`。阅读顺序与目录一致；**每一节都采用「概念说明 → 子工程真实代码 → 再说明」**，不放置占位符注释或单行碎片代码。十二章节的标题与 `Blog/framework-guides/_meta/guide-toc/generic-backend.yaml` 的 essentials 对齐，便于你在多栈仓库里横向对比其它后端指南而无需改自己的笔记结构。若你把本指南打印出来，可在页边把每节对应的 `main.go` 行号随手写上，形成个人定制的双联本。
...
Kratos 示例可压缩成三句话：**khttp.Server**、**HandleFunc**、**kratos.App**；**RouterGroup 负责路径前缀与局部中间件复用**；**Context 负责把一次请求的所有 IO 与元数据串起来**。本基础篇刻意用同一示例子工程贯穿十二章节，避免读者在不同文件间跳转而丢失情境。你只要跟着目录顺序读，`main.go` 里的每一行都会在某一节再次出现并配上针对性的中文解释。
	...
	api := r.Group("/api")
	{
		api.GET("/health", func(w http.ResponseWriter, r *http.Request) {
			writeJSON(w, http.StatusOK, map[string]any{
				"ok":      true,
				"service": "framework-back-end-kratos",
			})
		})
```

当你把 `HOST/PORT` 设为 `release`（见后文「配置与环境变量」）时，Gin 会关闭部分调试输出并采用更适合生产的默认策略；与 Debug 模式相比，错误页与日志粒度可能不同，因此本地开发通常保留 Debug，线上流水线注入 Release。`//go:embed public/index.html` 把静态页编入变量 `indexHTML`，由根路径 `GET /` 用 `c.Data` 原样返回，这对「单二进制交付」非常友好。与同仓其它后端相比，本 Go 示例固定端口 **3009**、`service` 字段 **framework-back-end-kratos**，便于自动化脚本统一探测。
...
「第一个 HTTP 服务」在这里体现为：**监听地址拼装 → `r.Run(addr)` 启动**。`Run` 内部创建 HTTP Server 并阻塞；出错时返回 `error`，子工程用 `log.Fatal` 打出原因后退出。默认 `HOST` 为 `127.0.0.1`，`PORT` 为 `3009`，因此本机在浏览器打开 `http://127.0.0.1:3009/` 应看到嵌入说明页；同时终端会打印「呈现页」与 `/api` 前缀的友好提示，方便新人对照路径自测。
...
```go
	...
	log.Printf("Kratos HTTP 演示：呈现页 http://%s/  |  API 前缀 /api", addr)
	app.Run(); err != nil {
		log.Fatal(err)
	}
}
...
HTTP 路由由 khttp.Server 维护；**处理器**就是绑定在特定方法与路径上的闭包或函数。顶层用 `r.GET`、`r.POST` 注册；分组后用 `api.GET` 等，路径会与组前缀拼接。子工程示例覆盖：`GET /` 返回 HTML；`GET /api/health`、`GET /api/demo/lifecycle`、`GET /api/items`、`POST /api/items`；以及 `GET /api/box/inner`（带路由组局部中间件）。这与同仓其它后端的 `fetch` 心智模型一致：固定 `/api` 前缀，资源型路径用名词复数。
...
```go
	...
	api := r.Group("/api")
	{
		...
		api.GET("/demo/lifecycle", func(w http.ResponseWriter, r *http.Request) {
			c.Header("x-kratos-demo", "handler-after-middleware")
			writeJSON(w, http.StatusOK, map[string]any{
...
从浏览器 `fetch` 调用 API 时，请从 `http://127.0.0.1:3009/` 打开呈现页，避免 `file://` 协议导致的跨域与安全限制。需要上传文件或表单时，应改用 `multipart` 绑定系列 API（本示例未覆盖，但 Context 路径一致）。响应头若由中间件与 Handler 同时设置，注意后者是否会覆盖前者，以及 CORS `ExposeHeaders` 是否列出了前端需要读取的自定义头。
...
子工程的 JSON 契约简单清晰：`GET /api/health` 返回探针对象；`GET /api/demo/lifecycle` 返回说明性 JSON，并带 `kratosPipeline` 字符串数组描述处理链路；`GET /api/items` 返回 `{ "items": [...] }`；`POST /api/items` 在成功时返回 `{ "item": {...} }`。字段名与表格呈现约定一致：`id`、`title`、`createdAt` 由 `item` 结构体的 `json` 标签控制序列化。`demo/lifecycle` 还返回 `kratosHighlights` 数组，每个元素是 `title` + `detail` 的键值对象，用来对照学习 **HTTP 路由器**、**Context**、**binding**、**并发** 四个高频主题。把「探针接口」与「教学内容接口」拆在不同路径上，是微服务与多栈仓库常用的做法：前者给编排系统看，后者给人看。
...
子工程读取 **`HOST/PORT`**、**`HOST`**、**`PORT`**。若未设置 `HOST/PORT`，代码显式调用 `r := mux.NewRouter()`，保证本地开箱即用；当你在生产容器注入 `生产环境变量（见官方文档）` 时，将跳过这段默认赋值路径并采用 Release。`HOST` 默认为 `127.0.0.1`，适合本机开发；在服务器上常改为 `0.0.0.0` 以便容器端口映射或反代访问。`PORT` 默认为 **3009**，与 Framework 端口约定一致；在 PowerShell 可临时 `$env:PORT="3011"` 验证多实例编排。

除此之外，建议在团队层面约定「环境变量命名空间」：公共前缀可以避免与系统或其他语言进程冲突；敏感配置不进仓库，不进日志。若你把 `PORT` 交给平台注入，也要在 README 写明健康检查的完整 URL，避免文档写 `3009` 而实际跑在 `8080`。对多区域部署，有时还要区分 `HOST/PORT` 与真正的「业务环境」（staging、prod），二者不要混用一个变量承载所有语义，否则你在排查问题时很难判断到底是框架日志级别变了还是下游依赖地址变了。
...
调试 HTTP 服务时，先把链路拆成「进程是否启动、端口是否可达、路由是否匹配、绑定是否成功、中间件顺序是否符合预期」。PowerShell 的 `Invoke-RestMethod` 对 JSON 友好；查看响应头可用 `Invoke-WebRequest`，关注 `x-kratos-demo`、`X-Feature-Box` 等自定义字段是否按路由组出现。子工程启动日志会打印根地址与 `/api` 前缀，便于复制到浏览器。
...
```powershell
Invoke-RestMethod http://127.0.0.1:3009/api/health | ConvertTo-Json
Invoke-RestMethod http://127.0.0.1:3009/api/items | ConvertTo-Json
Invoke-RestMethod http://127.0.0.1:3009/api/demo/lifecycle | ConvertTo-Json
(Invoke-WebRequest http://127.0.0.1:3009/api/box/inner).Headers
```
...
**transport/http 与 kratos.App 生命周期**

Kratos 强调 **transport** 层与 **App** 组合：HTTP Server 可与 gRPC Server 一同挂在 `kratos.New` 返回的 App 上。本子工程只演示 HTTP 一侧，但 `app.Run()` 仍展示统一启动与信号处理。`khttp.NewServer(khttp.Address(addr))` 中的地址默认可靠 **3009**；生产环境请把监听地址、超时与 TLS 配置纳入运维清单，而不是散落在 Handler 里。

**排障备忘**：若 `app.Run()` 立即退出，检查是否未正确构造 `kratos.Server` 切片。`writeJSON` 与探针字段 `framework-back-end-kratos` 必须保持稳定，供多栈脚本探测。与 go-zero 同场并行时，注意 **3008** 与 **3009** 不要混用。阅读官方文档时，区分「框架内置中间件」与「net/http 层中间件」，避免在错误层级查找钩子。
...
完整 `main.go`（`F:\Study\Framework\Back-end\Go\Kratos\main.go`）：
...
在制定个人学习计划时，把「能复现」放在「能讲清楚」之前：很多初学者过早沉迷于对比基准测试数据，却连一次完整的中间件链条都画不出来。反过来，已经有多年 Web 经验的读者，反而要小心「经验套用」——Kratos 并不是魔法，许多问题仍然是 HTTP 语义、序列化与并发错误，框架只是让你更快写出一致的结构。建议把本指南与官方文档交叉阅读：指南负责串起示例与叙述，文档负责查漏补缺与版本细节，两者合在一起才构成可持续的知识库。
...
- [Framework 子工程：Back-end/Go/Kratos](https://github.com/zhk0567/Framework/tree/main/Back-end/Go/Kratos)