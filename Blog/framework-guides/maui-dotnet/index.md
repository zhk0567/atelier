---
title: ".NET MAUI 官方指南：从入门到 Framework 子工程实战"
series: framework
category: Front-end
stack: MAUI
repo_path: Front-end/DotNet-Maui
...
# .NET MAUI 官方指南：从入门到 Framework 子工程实战
...
**.NET MAUI** 是跨平台原生 UI 框架（XAML + C#）。本目录**不提交**完整 `dotnet new maui` 树，提供 **`global.json`** 与生成说明；请在本机按 `MAUI-DotNet.md` 生成可运行工程。
...
**Framework 约定**：`GET /api/health` 返回 `service: framework-front-end-maui`；默认监听 **http://127.0.0.1:（本机生成）/**。与同仓 Node 示例对照时，请固定使用本文 PowerShell 路径与端口，便于脚本批量探活。呈现页 `public/index.html` 通过 `//go:embed` 编入二进制，部署时不必再单独拷贝静态目录。若你在内部 Wiki 引用本文，请同步贴上子工程仓库链接，避免同事只读到指南却找不到源码。
...
> **TIP**：本机并行跑多个 Go 示例时，用 `$env:PORT` 区分进程，避免与同仓 Blazor Server（5211）、Flutter等端口冲突。
...
> **预备知识**：.NET SDK **9+** 与 MAUI 工作负载（以子工程 `package.json` 的 `go` 指令为准）；理解 HTTP 方法、JSON、常见状态码（如 200、201、400）；会在 PowerShell 使用 `Set-Location -LiteralPath` 进入含 `package.json` 的目录；知道「中间件先执行、`c.Next()` 进入链条下游、返回后再回到上游」这一基本顺序。若你准备深入 `binding` 标签，建议预先了解 struct tag 与「校验失败时 Gin 如何格式化错误」之间的边界；若你只跑示例，可先忽略 validator 的高级规则，把精力放在路由组与上下文方法上。
...
以下各节对照 `F:\Study\Framework\Front-end\DotNet-Maui\global.json` 与 `package.json`。阅读顺序与目录一致；**每一节都采用「概念说明 → 子工程真实代码 → 再说明」**，不放置占位符注释或单行碎片代码。十二章节的标题与 `Blog/framework-guides/_meta/guide-toc/generic-backend.yaml` 的 essentials 对齐，便于你在多栈仓库里横向对比其它后端指南而无需改自己的笔记结构。若你把本指南打印出来，可在页边把每节对应的 `global.json` 行号随手写上，形成个人定制的双联本。
...
Gin 的核心抽象可以压缩成三句话：**Engine 负责承载中间件与路由树**；**RouterGroup 负责路径前缀与局部中间件复用**；**Context 负责把一次请求的所有 IO 与元数据串起来**。本基础篇刻意用同一示例子工程贯穿十二章节，避免读者在不同文件间跳转而丢失情境。你只要跟着目录顺序读，`global.json` 里的每一行都会在某一节再次出现并配上针对性的中文解释。
	...
	api := r.Group("/api")
	{
		api.GET("/health", func(w http.ResponseWriter, r *http.Request) {
			c.JSON(http.StatusOK, gin.H{
				"ok":      true,
				"service": "framework-front-end-maui",
			})
		})
```

当你把 `HOST/PORT` 设为 `release`（见后文「配置与环境变量」）时，Gin 会关闭部分调试输出并采用更适合生产的默认策略；与 Debug 模式相比，错误页与日志粒度可能不同，因此本地开发通常保留 Debug，线上流水线注入 Release。`//go:embed public/index.html` 把静态页编入变量 `indexHTML`，由根路径 `GET /` 用 `c.Data` 原样返回，这对「单二进制交付」非常友好。与同仓其它后端相比，本 MAUI 示例固定端口 **（本机生成）**、`service` 字段 **framework-front-end-maui**，便于自动化脚本统一探测。
...
子工程是独立 Go Module，模块路径在 `package.json` 的 `module` 行声明。依赖只声明直接引用的库，其余由 `dotnet workload install maui` 计算并写入 `go.sum`。公司网络若访问 proxy.golang.org 不稳定，可在 PowerShell 会话级别设置 `GOPROXY`（例如企业镜像），**不要**在指南仓库根目录执行 `go run`，必须在带子工程的目录操作。Windows 路径含空格或特殊字符时务必使用 `Set-Location -LiteralPath`。本示例不依赖 Node；前端呈现页只是嵌入的 HTML。
...
Go 模块系统锁定「依赖从哪来、版本如何解析」，但不替你决定项目分层是否清晰；因此即便 `dotnet workload install maui` 始终干净，也要警惕路由与业务逻辑搅在同一文件里难以测试。`go.sum` 应该进入版本控制，以抵御依赖投递过程中的篡改；内网私有模块要正确设置 `GOPRIVATE`，否则公共代理可能尝试抓取你公司的 import 路径。CI 与开发者本机的 `GOPROXY` 策略不一致时，最常见的症状是 checksum 对不齐，解决思路是统一代理与公司镜像，而不是在本地随意 `-insecure`。对 Windows 团队而言，路径分隔与脚本工具链有时会让新手误以为「Go 找不到模块」，多数情况下只是当前工作目录不对。

把环境准备当成「可复现构建」的一部分：`go version` 应不低于 1.21；`dotnet workload install maui` 无报错后再 `dotnet new maui -n App -o ./generated && cd generated && dotnet run`。若你 fork 了子工程并改了 module 路径，记得同步替换 import 的根路径与 `require` 列表，但**本指南引用的源码以当前 `global.json` 为准**。IDE 应能识别 `gin.Default`、`c.JSON` 等符号跳转；若不能，检查是否打开了含 `package.json` 的文件夹而非上一级 monorepo 根。
...
「第一个 HTTP 服务」在这里体现为：**监听地址拼装 → `r.Run(addr)` 启动**。`Run` 内部创建 HTTP Server 并阻塞；出错时返回 `error`，子工程用 `log.Fatal` 打出原因后退出。默认 `HOST` 为 `127.0.0.1`，`PORT` 为 `（本机生成）`，因此本机在浏览器打开 `http://127.0.0.1:（本机生成）/` 应看到嵌入说明页；同时终端会打印「呈现页」与 `/api` 前缀的友好提示，方便新人对照路径自测。
...
从浏览器 `fetch` 调用 API 时，请从 `http://127.0.0.1:（本机生成）/` 打开呈现页，避免 `file://` 协议导致的跨域与安全限制。需要上传文件或表单时，应改用 `multipart` 绑定系列 API（本示例未覆盖，但 Context 路径一致）。响应头若由中间件与 Handler 同时设置，注意后者是否会覆盖前者，以及 CORS `ExposeHeaders` 是否列出了前端需要读取的自定义头。
...
对照 Node 生态：Express、Fastify 的 `next()`、Koa 的 `await next()` 与 Gin 的 `c.Next()` 承担同构职责，但 Gin 的中间件签名统一、与路由树耦合更紧，迁移时注意不要把「异步前置逻辑」写进会阻塞链条的代码路径。下面节选包含全局 CORS 与 `/api/box` 子树的局部 Header 中间件，和 `global.json` 保持一致。
...
子工程读取 **`HOST/PORT`**、**`HOST`**、**`PORT`**。若未设置 `HOST/PORT`，代码显式调用 `（本栈无 Gin 式 SetMode）(gin.DebugMode)`，保证本地开箱即用；当你在生产容器注入 `HOST/PORT=release` 时，将跳过这段默认赋值路径并采用 Release。`HOST` 默认为 `127.0.0.1`，适合本机开发；在服务器上常改为 `0.0.0.0` 以便容器端口映射或反代访问。`PORT` 默认为 **（本机生成）**，与 Framework 端口约定一致；在 PowerShell 可临时 `$env:PORT="3011"` 验证多实例编排。

除此之外，建议在团队层面约定「环境变量命名空间」：公共前缀可以避免与系统或其他语言进程冲突；敏感配置不进仓库，不进日志。若你把 `PORT` 交给平台注入，也要在 README 写明健康检查的完整 URL，避免文档写 `（本机生成）` 而实际跑在 `8080`。对多区域部署，有时还要区分 `HOST/PORT` 与真正的「业务环境」（staging、prod），二者不要混用一个变量承载所有语义，否则你在排查问题时很难判断到底是框架日志级别变了还是下游依赖地址变了。
...
本地开发常见组合是：`HOST/PORT` 留空走 Debug，`PORT` 留给多实例切换，`HOST` 在需要局域网手机调试时临时改成 `0.0.0.0`。请避免把密钥直接写进 `global.json` 或 `package.json`，即便是示例子工程也要养成「密钥只来自环境」的肌肉记忆。若你在容器里同时挂多个配置文件，启动脚本要确保后加载层覆盖先加载层的规则写清楚，否则会出现「以为改了端口实际没生效」的乌龙。把环境变量的含义和示例写入团队 README，可以在数月后的深夜排障时节省大量猜测时间。
...
```powershell
Invoke-RestMethod http://127.0.0.1:（本机生成）/api/health | ConvertTo-Json
Invoke-RestMethod http://127.0.0.1:（本机生成）/api/items | ConvertTo-Json
Invoke-RestMethod http://127.0.0.1:（本机生成）/api/demo/lifecycle | ConvertTo-Json
(Invoke-WebRequest http://127.0.0.1:（本机生成）/api/box/inner).Headers
```
...
Android/iOS 构建需对应 SDK；Windows 可先 `dotnet build` 验证。

在进入 Framework 子工程实战全文之前，建议把 IDE 折叠到 `main` 函数与路由注册区，用「查找引用」跟踪 `itemsStore` 的读写点，确认 Mutex 覆盖所有变异路径。然后对照 `/api/demo/lifecycle` 返回的 pipeline 数组，在源码里为每一项标上行号；若 JSON 与代码对不上，说明本地运行的二进制不是当前目录编译产物，应重新 `dotnet new maui -n App -o ./generated && cd generated && dotnet run`。讲解给他人时，优先演示 health 与 POST items，再演示 box 子路由响应头，最后才打开呈现页；这一顺序符合「API 先通、UI 后验」的排障习惯。若你需要把示例改成 HTTPS 或反向代理后的路径前缀，请先固定 HTTP 形态下行为正确，再引入 TLS 与 `StripPrefix`，否则很难判断问题出在框架还是基础设施。Windows 终端若出现中文乱码，优先检查文件 UTF-8 与控制台代码页；若 `go test` 尚未编写，可先用 PowerShell 脚本做冒烟，但应尽快把关键路径迁到 `httptest`，以免框架升级时无人敢改路由表。最后提醒：占位工程（go-swagger、Revel 指南目录）与完整脚手架并存时，务必读 health JSON 里的 `note` 字段，不要对占位能力产生过高预期。团队分享时把终端字号调大，并在路由注册与中间件两行停顿，便于观众对照路径。若录制视频，在描述栏写上子工程路径、默认端口与 `service` 字段，方便观众复现。遇到疑难问题时，把 health、lifecycle、items 三条响应各保存一份，比只截图终端更有助于远程协助。改完代码务必重启进程；本示例未内置热重载，这是多数 Go HTTP 服务的常态。

**与 Framework 指南仓协作**：本页在 manifest 中保持 draft，评审通过前勿改为 published。若 Study 子工程 `global.json` 与指南不一致，优先修正子工程再同步本文。提 PR 时请附 health 与 items 的 PowerShell 输出，便于 Reviewer 五分钟内复现。汉字字数可用仓库脚本 `validate_guide.py --strict` 自查，质量规则用 `validate_guide_quality.py --strict`。评审时请勿在延伸阅读链接之后追加正文或代码块。

阅读完「部署概念」后，建议保留当前 PowerShell 会话：再次请求 `/api/demo/lifecycle`，把返回 JSON 当作本栈的「自述」，对照 `global.json` 标出每一句描述对应的代码行。若你在团队内做分享，可让听众仅根据 JSON 与响应头猜测框架，检验说明是否清晰。最后通读 Framework 子工程实战中的完整 `global.json`（带注释），在检查清单上逐项打勾，形成「能跑、能讲、能改」的闭环，再按需翻阅官方文档补全未展开的主题（ORM、配置中心、链路追踪等）。

## Framework 子工程实战

这一节把分散在基础篇里的接口一次性列成「方法与路径」表，并给出**依赖清单**与**带中文行注的完整 `global.json`**，方便你打印或投屏讲解。建议你按表格自上而下自测：先看探针是否返回约定的 `service` 字段，再验证列表与创建是否符合 REST 风格的状态码，然后关注路由组中间件是否只在子路径写入 `X-Feature-Box`。向同事演示时，可以一边打开 `global.json` 一边用 PowerShell 打请求，把「代码行」与「网络包」对应起来；新人最常卡住的地方是忘记重启进程或把工作目录开在仓库外层导致 `go:embed` 找不到文件，这两点在演示现场点名指出能显著缩短上手时间。若你要把示例子工程当作脚手架起点，优先复制「路由组 + 中间件 + 绑定校验」三板斧，再替换内存存储为真实持久层，而不是先引入一堆生成器把目录搅乱。

完整源码（`F:\Study\Framework\Front-end\DotNet-Maui`，主入口 `global.json`）：
...
| 路径 | 建议 |
|------|------|
| 零基础 | 导读 → 快速上手 → 基础篇顺序阅读 → 子工程实战通读带注 `global.json` |
...
无论哪条路径，都建议你在读完「参数与校验」后立刻停下来，用记事本写五个自己会犯的错：**Body 读两次**、**忘了 ExposeHeaders**、**在错误的 `Group` 上注册中间件**、**Release 模式却仍打印调试密钥**、**Mutex 未保护共享切片**。这五个错与示例子工程的扩展方向高度重合，提前写下来能显著减少夜里排障的时间。最后，把 `global.json` 当作活文档：注释只保留解释「为什么这样写」的句子，把「框架百科」留在本指南与官方文档里，更符合长期维护的剪裁原则。
...
- [Framework 子工程：Front-end/DotNet-Maui](https://github.com/zhk0567/Framework/tree/main/Front-end/DotNet-Maui)